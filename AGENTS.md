...
4. Accepts natural language queries, translates them to SQL via Codex, and returns grounded responses
...
| LLM | Anthropic Codex API (`Codex-sonnet-4-6`) |
...
│   ├── llm/                 # Codex API client, prompt templates, guardrails
...
User input → POST /api/query
  ...
  → llm/translator.py: schema-aware prompt → Codex → SQL string
  → query/validator.py: validate SQL (no destructive ops, restricted to read-only)
  → query/executor.py: execute against PostgreSQL
  → llm/responder.py: structured results → Codex → natural language summary
...
- All Codex calls go through `llm/client.py` — never call the Anthropic SDK directly from other modules
...
- Codex model: `Codex-sonnet-4-6` by default; configurable via `CLAUDE_MODEL` env var
